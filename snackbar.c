...
void add_items(void)
{
    // If we had the items in a separate array, we could use a for loop (i), to replace the element
    // position with each, and make the item and price equal to their respective products and
    // amounts.
    menu[0].item = "Burger";
    menu[0].price = 9.5;
...
float get_cost(string item)
{
    ...
    // If item not found, returns special value -> error message ...!
    return 0.00;
}
